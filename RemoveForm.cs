...
namespace InjectorInspector
{
    public partial class RemoveForm : Form
    {
        ...
        public BindingList<JsonRemoveModeContent> jsonContentList;

        // 在 Form 類中創建 apiJsonRemoveModeHandle 的實例
        public apiJsonRemoveModeHandle apiHandler;
        ...
        public void RemoveForm_Load(object sender, EventArgs e)
        {
            this.Text = "RemoveForm_L=123";

            //當表單開啟時，讀json檔案
            if(true)
            {
                // 初始化 apiJsonRemoveModeHandle，並指定檔案名稱
                if (apiHandler == null)
                {
                    // 如果 apiHandler 是 null，進行初始化
                    apiHandler = new apiJsonRemoveModeHandle();
                }

                // 指定檔案名稱並初始化 JSON 文件
                apiHandler.InitialJsonFile("SaveRemoveJason.json");

                // 使用 apiJsonRemoveModeHandle 中的 JsonNeedleContentList 資料
                jsonContentList = new BindingList<JsonRemoveModeContent>(apiHandler.JsonNeedleContentList);
        ...
        public void RemoveForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Text = "RemoveForm_L=456";

            form1.btn_取針.Enabled = true;
        }
    ...
    //Profile of RemoveMode 
    public class JsonRemoveModeContent
    {
        public string strLabel { get; set; }
        public uint u32Index { get; set; }
        public double dbPosition { get; set; }
        public string strNote { get; set; }
    }  //end of public class JsonRemoveModeContent

    //Handle of RemoveMode 
    public class apiJsonRemoveModeHandle
    {
        public List<JsonRemoveModeContent> JsonNeedleContentList { get; set; }
        ...
        public void InitialJsonFile(string strNameFile)
        {
            ...
            JsonNeedleContentList = new List<JsonRemoveModeContent>();
            GenerateFilePath();
            ...
            if (File.Exists(FilePath))
            {
                try
                {
                    ...
                    JsonNeedleContentList = JsonSerializer.Deserialize<List<JsonRemoveModeContent>>(jsonString) ?? new List<JsonRemoveModeContent>();
        ...
        public void AddJsonContent(JsonRemoveModeContent newContent, bool bForceWriteConflict)
        {
            if (newContent == null)
            {
            ...
            // 更新 JSON 文件
            WriteDataToJsonFile();
        }  // end of public void AddJsonContent(JsonRemoveModeContent newContent, bool bForceWriteConflict)
        ...
        public void UpdateJsonContentByIndex(uint u32Index, JsonRemoveModeContent updatedContent)
        {
        ...
        }  // end of public void UpdateJsonContentByIndex(uint u32Index, JsonRemoveModeContent updatedContent)

        public JsonRemoveModeContent ReadJsonContentByIndex(uint u32Index)
        {
            JsonRemoveModeContent jtmcRSLT = new JsonRemoveModeContent();
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(jsonString))
                {
                    ...
                    List<JsonRemoveModeContent> TestReadWriteJson = JsonSerializer.Deserialize<List<JsonRemoveModeContent>>(jsonString);

                    // 查找 u32_Index
                    JsonRemoveModeContent GotNeedle = TestReadWriteJson?.FirstOrDefault(p => p.u32Index == u32Index);
            ...
            return jtmcRSLT;
        }  //end of public JsonRemoveModeContent ReadJsonContentByIndex(uint u32Index)

    }  //end of public class apiJsonRemoveModeHandle
}
