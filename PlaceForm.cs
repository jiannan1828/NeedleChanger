...
namespace InjectorInspector
{
    public partial class PlaceForm : Form
    {
        ...
        public BindingList<JsonPlaceModeContent> jsonContentList;

        // 在 Form 類中創建 apiJsonPlaceModeHandle 的實例
        public apiJsonPlaceModeHandle apiHandler;
        ...
        public void PlaceForm_Load(object sender, EventArgs e)
        {
            this.Text = "PlaceForm_L=123";

            //當表單開啟時，讀json檔案
            if(true)
            {
                // 初始化 apiJsonPlaceModeHandle，並指定檔案名稱
                if (apiHandler == null)
                {
                    // 如果 apiHandler 是 null，進行初始化
                    apiHandler = new apiJsonPlaceModeHandle();
                }

                // 指定檔案名稱並初始化 JSON 文件
                apiHandler.InitialJsonFile("SavePlaceJason.json");

                // 使用 apiJsonPlaceModeHandle 中的 JsonNeedleContentList 資料
                jsonContentList = new BindingList<JsonPlaceModeContent>(apiHandler.JsonNeedleContentList);
        ...
        public void PlaceForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Text = "PlaceForm_L=456";

            form1.btn_植針.Enabled = true;
        }
    ...
    //Profile of PlaceMode 
    public class JsonPlaceModeContent
    {
        public string strLabel { get; set; }
        public uint u32Index { get; set; }
        public double dbPosition { get; set; }
        public string strNote { get; set; }
    }  //end of public class JsonPlaceModeContent

    //Handle of PlaceMode 
    public class apiJsonPlaceModeHandle
    {
        public List<JsonPlaceModeContent> JsonNeedleContentList { get; set; }
        ...
        public void InitialJsonFile(string strNameFile)
        {
            ...
            JsonNeedleContentList = new List<JsonPlaceModeContent>();
            GenerateFilePath();
            ...
            if (File.Exists(FilePath))
            {
                try
                {
                    ...
                    JsonNeedleContentList = JsonSerializer.Deserialize<List<JsonPlaceModeContent>>(jsonString) ?? new List<JsonPlaceModeContent>();
        ...
        public void AddJsonContent(JsonPlaceModeContent newContent, bool bForceWriteConflict)
        {
            if (newContent == null)
            {
            ...
            // 更新 JSON 文件
            WriteDataToJsonFile();
        }  // end of public void AddJsonContent(JsonPlaceModeContent newContent, bool bForceWriteConflict)
        ...
        public void UpdateJsonContentByIndex(uint u32Index, JsonPlaceModeContent updatedContent)
        {
        ...
        }  // end of public void UpdateJsonContentByIndex(uint u32Index, JsonPlaceModeContent updatedContent)

        public JsonPlaceModeContent ReadJsonContentByIndex(uint u32Index)
        {
            JsonPlaceModeContent jtmcRSLT = new JsonPlaceModeContent();
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(jsonString))
                {
                    ...
                    List<JsonPlaceModeContent> TestReadWriteJson = JsonSerializer.Deserialize<List<JsonPlaceModeContent>>(jsonString);

                    // 查找 u32_Index
                    JsonPlaceModeContent GotNeedle = TestReadWriteJson?.FirstOrDefault(p => p.u32Index == u32Index);
            ...
            return jtmcRSLT;
        }  //end of public JsonPlaceModeContent ReadJsonContentByIndex(uint u32Index)

    }  //end of public class apiJsonPlaceModeHandle
}
