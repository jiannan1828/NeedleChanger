...
{
    public partial class TakePinForm : Form
    {
        // 定義一個 BindingList 用來綁定到 DataGridView
        public BindingList<JsonTakePinModeContent> jsonContentList;

        // 在 Form 類中創建 apiJsonTakePinModeHandle 的實例
        public apiJsonTakePinModeHandle apiHandler;
        ...
        public void TakePinForm_Load(object sender, EventArgs e)
        {
            this.Text = "TakePinForm_L=123";

            //當表單開啟時，讀json檔案
            if(true)
            {
                // 初始化 apiJsonTakePinModeHandle，並指定檔案名稱
                if (apiHandler == null)
                {
                    // 如果 apiHandler 是 null，進行初始化
                    apiHandler = new apiJsonTakePinModeHandle();
                }

                // 指定檔案名稱並初始化 JSON 文件
                apiHandler.InitialJsonFile("SaveTakePinJason.json");

                // 使用 apiJsonTakePinModeHandle 中的 JsonNeedleContentList 資料
                jsonContentList = new BindingList<JsonTakePinModeContent>(apiHandler.JsonNeedleContentList);
        ...
        public void TakePinForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Text = "TakePinForm_L=456";

            form1.btn_拋料.Enabled = true;
        }
    ...
    //Profile of TakePinMode 
    public class JsonTakePinModeContent
    {
        public string strLabel { get; set; }
        public uint u32Index { get; set; }
        public double dbPosition { get; set; }
        public string strNote { get; set; }
    }  //end of public class JsonTakePinModeContent

    //Handle of TakePinMode 
    public class apiJsonTakePinModeHandle
    {
        public List<JsonTakePinModeContent> JsonNeedleContentList { get; set; }
        ...
        public void InitialJsonFile(string strNameFile)
        {
            ...
            JsonNeedleContentList = new List<JsonTakePinModeContent>();
            GenerateFilePath();
            ...
            if (File.Exists(FilePath))
            {
                try
                {
                    ...
                    JsonNeedleContentList = JsonSerializer.Deserialize<List<JsonTakePinModeContent>>(jsonString) ?? new List<JsonTakePinModeContent>();
        ...
        public void AddJsonContent(JsonTakePinModeContent newContent, bool bForceWriteConflict)
        {
            if (newContent == null)
            {
            ...
            // 更新 JSON 文件
            WriteDataToJsonFile();
        }  // end of public void AddJsonContent(JsonTakePinModeContent newContent, bool bForceWriteConflict)
        ...
        public void UpdateJsonContentByIndex(uint u32Index, JsonTakePinModeContent updatedContent)
        {
        ...
        }  // end of public void UpdateJsonContentByIndex(uint u32Index, JsonTakePinModeContent updatedContent)

        public JsonTakePinModeContent ReadJsonContentByIndex(uint u32Index)
        {
            JsonTakePinModeContent jtmcRSLT = new JsonTakePinModeContent();
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(jsonString))
                {
                    ...
                    List<JsonTakePinModeContent> TestReadWriteJson = JsonSerializer.Deserialize<List<JsonTakePinModeContent>>(jsonString);

                    // 查找 u32_Index
                    JsonTakePinModeContent GotNeedle = TestReadWriteJson?.FirstOrDefault(p => p.u32Index == u32Index);
            ...
            return jtmcRSLT;
        }  //end of public JsonTakePinModeContent ReadJsonContentByIndex(uint u32Index)

    }  //end of public class apiJsonTakePinModeHandle
}
